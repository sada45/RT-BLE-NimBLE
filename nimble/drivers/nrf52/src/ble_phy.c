...
uint32_t total_tx_time;
uint32_t tx_start_time;

uint32_t total_rx_time;
...
ble_phy_set_start_time(uint32_t cputime, uint8_t rem_usecs, bool tx)
{
    ...
    if (tx) {
		tx_start_time = os_cputime_ticks_to_usecs(cputime) + rem_usecs;
    }
    else {
		rx_start_time = os_cputime_ticks_to_usecs(cputime) + rem_usecs;
    }
...
ble_phy_set_start_now(void)
{
    ...
    /* RT-BLE: only Rx will call this func */
    rx_start_time = os_cputime_ticks_to_usecs(now);
...
ble_phy_disable(void)
{
    ...
    ble_phy_disable_irq_and_ppi();
	if (rx_start_time > 0) {
		total_rx_time += (os_cputime_ticks_to_usecs(os_cputime_get32()) - rx_start_time);
		rx_start_time = 0;
	}
	if (tx_start_time > 0) {
		total_tx_time += (os_cputime_ticks_to_usecs(os_cputime_get32()) - tx_start_time);
		tx_start_time = 0;
	}
}

void ble_phy_clear(void)
{
	total_tx_time = 0;
	total_rx_time = 0;
}

void ble_phy_print_radio_time(void)
{
	rtt_printf(",tx_time=");
	rtt_print_uint32(total_tx_time);
	rtt_printf(",rx_time=");
	rtt_print_uint32(total_rx_time);
}
...
void ble_phy_output_duty_cycle(void)
{
    printf(",tx_time=%lu,rx_time=%lu", total_tx_time, total_rx_time);
    // rtt_print_uint32(total_tx_time);
    // rtt_printf(",rx_time=");
    // rtt_print_uint32(total_rx_time);
}