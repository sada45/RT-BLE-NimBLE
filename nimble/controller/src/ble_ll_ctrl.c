...
#if RTBLE || BLEX
	/* RT-BLE: If there is an pending subrate update request,
	 * we take the subrate factor to 1 */
	if (connsm->cur_ctrl_proc == BLE_LL_CTRL_PROC_SUBRATE_UPDATE) {
		instant = connsm->event_cntr + 6 * (connsm->periph_latency + 1);
	}
	else {
...
#endif
#else
	/* RT-BLE: we set the the instant to the minimal 6 CEs */
    instant = connsm->event_cntr + connsm->periph_latency + 6;
#endif
                       ...
                       void *data)
{
    ...
    om = NULL;
#if RTBLE
	/* RT-BLE: if there is a ongonging subrating process and this is the connection
	 * update process, we can make and enqueue this packet into the tx buff.
	 */
    if (connsm->cur_ctrl_proc == BLE_LL_CTRL_PROC_IDLE ||
    	(connsm->cur_ctrl_proc == BLE_LL_CTRL_PROC_SUBRATE_UPDATE
    		&& ctrl_proc == BLE_LL_CTRL_PROC_CONN_UPDATE) ||
        (connsm->cur_ctrl_proc == BLE_LL_CTRL_PROC_CONN_UPDATE
    		&& ctrl_proc == BLE_LL_CTRL_PROC_SUBRATE_UPDATE)) {
#else
	if (connsm->cur_ctrl_proc == BLE_LL_CTRL_PROC_IDLE) {
#endif
