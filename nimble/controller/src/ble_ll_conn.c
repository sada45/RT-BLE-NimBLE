...
// Some modification to implement RTBLE
sema_t output_sem;
sema_t txdone_sema;
sema_t tx_start_sema;
uint32_t tx_end_us;
uint32_t start_tx_time;
uint8_t last_hdr;
uint32_t last_pkt_recved_time;
ble_ll_connected_cb ll_connected_cb;

#if RTBLE || BLEX
uint32_t base_tick;
extern int8_t tree_level;
extern uint32_t pow164[];
sema_t conn_upd_sema;
#endif

#if RTBLE
ble_subrate_done_callback subrate_cb;
#endif
...
{
    struct ble_ll_conn_sm *connsm;
    struct ble_ll_conn_sm *target_connsm;
    uint16_t min_conn_handle = MYNEWT_VAL(BLE_L2CAP_COC_MAX_NUM);
	if (STAILQ_EMPTY(&g_ble_ll_conn_free_list)) {
		STATS_INC(ble_ll_conn_stats, no_free_conn_sm);
		return NULL;
	}
    STAILQ_FOREACH(connsm, &g_ble_ll_conn_free_list, free_stqe) {
		if (connsm->conn_handle <= min_conn_handle) {
			min_conn_handle = connsm->conn_handle;
			target_connsm = connsm;
		}
    }
    STAILQ_REMOVE(&g_ble_ll_conn_free_list, target_connsm, ble_ll_conn_sm, free_stqe);
    return target_connsm;
}
...
{
	struct ble_ll_conn_sm *connsm = (struct ble_ll_conn_sm *)arg;
#if TXEND_OUTPUT
	if (!CONN_F_EMPTY_PDU_TXD(connsm)) {
		tx_end_us = os_cputime_ticks_to_usecs(os_cputime_get32());
        last_hdr = connsm->last_hdr_byte;
		sema_post(&output_sem);
		// if (connsm->last_hdr_byte & BLE_LL_MANUALPKT_LOSS_MASK) {
		// 	rtt_printf(",loss_txend=");
		// }
		// else {
		// 	rtt_printf(",txend=");
		// }
		// rtt_print_uint32(tx_end_us);
	}
#endif
...
//RT-BLE: for log output
static void
ble_ll_conn_txend_output(void *arg)
{
#if TXEND_OUTPUT
	struct ble_ll_conn_sm *connsm = (struct ble_ll_conn_sm *)arg;
	if (!CONN_F_EMPTY_PDU_TXD(connsm)) {
...
void ble_ll_conn_wait_output_sem(void)
{
	sema_wait(&output_sem);
    if (last_hdr & BLE_LL_MANUALPKT_LOSS_MASK) {
        printf(",loss_txend=%lu", tx_end_us);
    }
    else {
        printf(",txend=%lu", tx_end_us);
    }
	// rtt_printf(",txend=");
	// rtt_print_uint32(tx_end_us);
//	printf(",%lu", tx_end_us);
...
void ble_ll_conn_wait_start_tx_sema(void)
{
    sema_wait(&tx_start_sema);
    printf(",start_tx=%lu", start_tx_time);
}

void ble_ll_sema_consume(void)
{
	while (output_sem.value > 0) {
		sema_try_wait(&output_sem);
	}
    while (tx_start_sema.value > 0) {
        sema_try_wait(&tx_start_sema);
    }
}
...
ble_ll_conn_get_next_sched_time(struct ble_ll_conn_sm *connsm)
{
    ...
    uint8_t rem_us;
    int rc;
    /* RT-BLE: we can assign multiple slots to a connection, and may exceed the
     * connection interval (10ms). When check wethere we can send on the next event, 
     * we only consider the next connection event which is not belong to current connection
     * The peripheral do nott need to know anything, because if the central sends a packet with 
     * MD=1, it alway relpy in next data exchange. And it will check wether the start time of next
     * connection event is pass the current cputime.
     */
    rc = ble_ll_sched_next_time(&next_sched_time);
    if (connsm->cont_num > 0) {
        if (rc) {
            return next_sched_time;
        }
        else {
            /* There is no other connection, we just simply set the length up to 20ms */
            ce_end = connsm->anchor_point + 2 * connsm->conn_itvl_ticks -
...
#if RTBLE
		if (subrate_cb && connsm->subrate_sema == 1) {
			// rtt_printf("recv subrate = ");
			// rtt_print_uint32(os_cputime_ticks_to_usecs(os_cputime_get32()));
			// rtt_printf("\n");
			subrate_cb(connsm->conn_handle);
		}
#endif
    }
...
#if TXEND_OUTPUT
    /* The last is empty and lost. but now we have data to send */
    if ((pkthdr || connsm->cur_tx_pdu) && CONN_F_EMPTY_PDU_TXD(connsm)) {
		rtt_printf(",delay=");
		rtt_print_uint32(os_cputime_ticks_to_usecs(os_cputime_get32()));
    }
#endif
...
//    if (connsm->cur_tx_pdu) {
//		ble_hdr = BLE_MBUF_HDR_PTR(connsm->cur_tx_pdu);
//		if ((ble_hdr->txinfo.hdr_byte & BLE_LL_DATA_HDR_LLID_MASK)
//                    == BLE_LL_LLID_CTRL) {
//			rtt_printf("ctrl tx time = ");
//			rtt_print_uint32(os_cputime_get32());
//			rtt_printf("\n");
//         }
//    }
...
conn_tx_pdu:
	...
	/* RT-BLE: if this is data packet, we introuce manual packet loss */
#if CONTINUOUS_LOSS
    /* It is the last data pdu, we drop then */
    if (connsm->conn_role == CUR_TEST_ROLE && 
        connsm->cur_tx_pdu && STAILQ_FIRST(&connsm->conn_txq) == NULL) {
        llid = ble_hdr->txinfo.hdr_byte & BLE_LL_DATA_HDR_LLID_MASK;
        if (llid != BLE_LL_LLID_CTRL && connsm->loss_num_left > 0) {
           hdr_byte |= BLE_LL_MANUALPKT_LOSS_MASK; 
           connsm->loss_num_left--;
        }
    }
#else
	if (!CONN_F_EMPTY_PDU_TXD(connsm) && llid != BLE_LL_LLID_CTRL && ((rand() % 100) < PKT_LOSS_RATE)) {
		hdr_byte |= BLE_LL_MANUALPKT_LOSS_MASK;
	}
#endif
    ...
    /* RT-BLE: we don't need to modify the peripheral
     * Since it will always reply if the central has more data to send */
    if ((connsm->csmflags.cfbit.terminate_ind_rxd) ||
        ...
        txend_func = ble_ll_conn_txend_output;
    }
...
#if TXEND_OUTPUT
   	connsm->last_hdr_byte = hdr_byte;
	if (!CONN_F_EMPTY_PDU_TXD(connsm)) {
        
        sema_post(&tx_start_sema);
	   	// rtt_printf(",start_tx=");
	   	// rtt_print_uint32(os_cputime_ticks_to_usecs(os_cputime_get32()));
   	}
#endif
...
#if TXEND_OUTPUT
    start_tx_time = os_cputime_ticks_to_usecs(os_cputime_get32());
#endif
...
#if RTBLE || BLEX
		/* RT-BLE: we update the base tick so the other connection can
		 * find the offset
		 */
		 if (base_tick > 0 && tree_level >= 0){
#if OUTPUT_ANCHOR
//		 	rtt_printf("=============\ntime set base = ");
//		 	rtt_print_uint32(os_cputime_get32());
#endif
		 	while ((int32_t)(sch->start_time - base_tick - pow164[tree_level]) >= 0) {
				base_tick += pow164[tree_level];
#if OUTPUT_ANCHOR
		 		rtt_printf("set base to = ");
		 		rtt_print_uint32(base_tick + g_ble_ll_sched_offset_ticks);
		 		rtt_printf("\n");
#endif
			}
		 }
                              ...
                              uint32_t add_usecs)
{
    ...
    if (connsm->conn_role == BLE_LL_CONN_ROLE_CENTRAL) {
        ...
        /* RT-BLE: we do not transmit the MIC, so the next packet the peripheral can send is 261 bytes
         * Since the central can know how mach data the peripheral can send, the 2088us can be replaced
         * with a more accurate number. For example, if the peripheral has no data to send, the 2088us can be changed to 0us 
         */
#if RTBLE
		usecs += (BLE_LL_IFS * 2) + connsm->peripheral_us;
#else 
        usecs += (BLE_LL_IFS * 2) + 2088;
#endif
...
#if RTBLE || BLEX
    connsm->offset = cc_params->offset;
#endif
}
#endif
#if (BLE_LL_BT5_PHY_SUPPORTED == 1)
...
ble_ll_conn_sm_new(struct ble_ll_conn_sm *connsm)
{
    ...
    connsm->connected_flag = 0;

#if RTBLE
    connsm->cur_mode = 0;
    connsm->is_continuation = 0;
    connsm->level = 0;
    connsm->slot_num = 0;
    connsm->critical_level = 0;
    connsm->critical_offset = 0;
    connsm->critical_slot_num = 0;
    connsm->subrate_anchor_point = 0;
#if CONTINUOUS_LOSS
    connsm->loss_num_left = CONTINUOUS_LOSS_NUM;
#endif
#endif 

#if BLEX
    connsm->expect_conn_event_len = 0.0f;
#endif
#if MYNEWT_VAL(BLE_LL_CFG_FEAT_LL_ENHANCED_CONN_UPDATE)
...
ble_ll_conn_next_event(struct ble_ll_conn_sm *connsm)
{
    ...
    if (connsm->cont_num_left > 0) {
        ...
        next_is_subrated = 0;
#if RTBLE
        if (connsm->slot_num > 0 && (connsm->event_cntr + 1 - base_event_cntr) > (connsm->slot_num - 1) / 2) {
            next_is_subrated = 1;
        }
        else {
            connsm->is_continuation = 1;
        }
#endif
    }
...
#if RTBLE || BLEX
			connsm->anchor_point += connsm->conn_itvl_ticks * event_cntr_diff;
#else
     ...
     */
    /*
     * RT-BLE: We move the code here, since we need to update the subrate anchor point
     */
    if (next_is_subrated ||
...
#if RTBLE
		connsm->subrate_anchor_point = connsm->anchor_point;
#endif
    }
#endif
    ...
    if (connsm->csmflags.cfbit.conn_update_sched &&
        ...
        connsm->conn_itvl = upd->interval;
#if RTBLE || BLEX
		connsm->conn_itvl_ticks = connsm->conn_itvl * 41;
        ...
        if (upd->winoffset != 0) {
#if RTBLE || BLEX
			connsm->anchor_point += upd->winoffset * 41;
#else
...
#if RTBLE
	    /*
		 * RT-BLE: the next connection event is the instant,
		 * we should info the rt-ble module to transmit the subrating ind
		 * Here, the anchor point of the connsm should be updated to the next
		 */
		sema_post(&conn_upd_sema);
#endif
...
#if MYNEWT_VAL(BLE_LL_CFG_FEAT_LL_ENHANCED_CONN_UPDATE) && RTBLE
    /*
     * RT-BLE: We move the code here, since we need to update the subrate anchor point
     */
    if (next_is_subrated ||
...
#endif
	/*
	 * RT-BLE: when the central received the first ack, this connection is considered
	 * as established
	 */
#if MYNEWT_VAL(BLE_LL_CFG_FEAT_LL_ENHANCED_CONN_UPDATE)
	if (connsm->conn_role == BLE_LL_CONN_ROLE_CENTRAL && ll_connected_cb
		&& connsm->connected_flag == 1) {
		ll_connected_cb(connsm->conn_handle);
		connsm->connected_flag |= 2;
	}
#endif

#if BLEX
    /* Here we modified the original BLEX design 
     * The original policy will take all conenction event into consideration, no matter it has data or not
     * It works fine in its evaluation but badly in our evaluation
     * To ensure faireness, we modified it so it only re-estimate connection event length when there is data
     */
	if (connsm->has_nonempty_pdu && connsm->cons_rxd_bad_crc < 2 && connsm->last_pdu_event == connsm->event_cntr) {
		if (connsm->expect_conn_event_len == 0.0f) {
			connsm->expect_conn_event_len = (double)(cur_cputime - connsm->anchor_point + 7);
		}
		else {
			connsm->expect_conn_event_len = 0.5 * connsm->expect_conn_event_len +
											0.5 * (double)(cur_cputime - connsm->anchor_point + 7.0f);
		}
	}
#endif
...
//    rtt_printf("\ncur_cputime = ");
//    rtt_print_uint32(os_cputime_get32());
	rtt_printf("conn_handle = ");
	rtt_print_uint32((uint32_t)connsm->conn_handle);
	rtt_printf(", next anchor = ");
	rtt_print_uint32(connsm->anchor_point);
    rtt_printf(", next us = ");
    rtt_print_uint32(os_cputime_ticks_to_usecs(connsm->anchor_point));
	rtt_printf(", cntr = ");
	rtt_print_uint32((uint32_t)connsm->event_cntr);
	rtt_printf("\n");
#endif
    /*
...
#endif
#if RTBLE || BLEX
	if (ble_ll_sched_conn_central_new(connsm, rxhdr, 0, base_tick)) {
		return -1;
	}
#else
...
ble_ll_conn_rx_data_pdu(struct os_mbuf *rxpdu, struct ble_mbuf_hdr *hdr)
{
    ...
    connsm->has_nonempty_pdu = 1;
#endif
    // /* This PDU has data, we update the last_data_event */
    // connsm->last_data_event = connsm->event_cntr;
    if (llid == BLE_LL_LLID_CTRL) {
...
ble_ll_conn_rx_isr_end(uint8_t *rxbuf, struct ble_mbuf_hdr *rxhdr)
{
    ...
    if (!BLE_MBUF_HDR_CRC_OK(rxhdr) ||
    	(BLE_MBUF_HDR_CRC_OK(rxhdr) && (hdr_byte & BLE_LL_MANUALPKT_LOSS_MASK))) {
        alloc_rxpdu = false;
    }
    ...
    if (!BLE_MBUF_HDR_CRC_OK(rxhdr) ||
        ...
        if (CONN_F_EMPTY_PDU_TXD(connsm) || connsm->cur_tx_pdu) {
            ...
            if ((hdr_nesn && conn_sn) || (!hdr_nesn && !conn_sn)) {
                ...
                /* Transmit success */
				connsm->connected_flag |= 1;
				// May have to set this too to cancel the empty connection
                connsm->tx_seqnum ^= 1;
                ...
                if (txpdu) {
                    ...
                    if (connsm->conn_role == CUR_TEST_ROLE && 
                        connsm->cur_tx_pdu && STAILQ_FIRST(&connsm->conn_txq) == NULL) {
                        if ((txhdr->txinfo.hdr_byte & BLE_LL_DATA_HDR_LLID_MASK)!= BLE_LL_LLID_CTRL && connsm->loss_num_left == 0) {
                            /* The packet is correctly received, we reset the loss_num_left */
                            connsm->loss_num_left = CONTINUOUS_LOSS_NUM;
                        }
                    }
#endif
                ...
                if (has_data_flag && connsm->cur_tx_pdu == NULL && STAILQ_FIRST(&connsm->conn_txq) == NULL) {
                    last_pkt_recved_time = os_cputime_ticks_to_usecs(os_cputime_get32());
                    sema_post(&txdone_sema);
                }
            }
        }
                        ...
                        uint8_t hdr_byte, uint16_t length)
{
    ...
    if (connsm->conn_role == BLE_LL_CONN_ROLE_PERIPHERAL) {
	    ...
	    // rtt_print_uint32(os_cputime_ticks_to_usecs(os_cputime_get32()));
// #if BLEX
//         uint32_t next_anchor = connsm->anchor_point;
//         uint32_t itvl_tick = connsm->conn_itvl * 41;
//         uint32_t enqueue = os_cputime_get32();
//         while (next_anchor - itvl_tick > enqueue) {
//             next_anchor -= itvl_tick;
//         }
//         printf(",enqueue_next_anchor=%lu", os_cputime_ticks_to_usecs(next_anchor));
// #endif
// #if RTBLE
//         uint32_t next_anchor = connsm->subrate_anchor_point;
//         uint32_t itvl_tick = connsm->conn_itvl * connsm->subrate_factor * 41;
//         uint32_t enqueue = os_cputime_get32();
//         while (next_anchor - itvl_tick > enqueue) {
//             next_anchor -= itvl_tick;
//         }
...
void
ble_ll_conn_set_connected_cb(ble_ll_connected_cb cb)
{
	ll_connected_cb = cb;
}

int ble_ll_conn_active_connsm_num(void)
{
	struct ble_ll_conn_sm *connsm;
	int active_num = 0;
	SLIST_FOREACH(connsm, &g_ble_ll_conn_active_list, act_sle) {
...
int ble_ll_conn_wait_tx_done(void) 
{
    sema_wait(&txdone_sema);
    printf(",last_recv_time=%lu", last_pkt_recved_time);
    return 0;
}

int ble_ll_conn_clear_tx_done_sema(void)
{
    if (txdone_sema.value > 0) {
        sema_try_wait(&txdone_sema);
    }
    return 0;
}

#if BLEX
int blex_move_anchor(struct ble_ll_conn_sm *connsm, int offset)
{
	...
	if (offset >= connsm->conn_itvl || offset <= -connsm->conn_itvl) {
		return -3;
	}

	// Check wether there are ongoing ctrl proc
	if (IS_PENDING_CTRL_PROC(connsm, BLE_LL_CTRL_PROC_CONN_PARAM_REQ) ||
	  IS_PENDING_CTRL_PROC(connsm, BLE_LL_CTRL_PROC_CONN_UPDATE)) {
	...
	hcu->conn_latency = connsm->periph_latency;
	hcu->supervision_timeout = connsm->supervision_tmo;
	hcu->min_ce_len = connsm->min_ce_len;
	hcu->max_ce_len = connsm->max_ce_len;
	hcu->handle = connsm->conn_handle;
	if (offset > 0) {
		hcu->winoffset = (uint16_t)offset;
	}
	else{
		hcu->winoffset = (uint16_t)(connsm->conn_itvl + offset);
	}
	while (sema_try_wait(&conn_upd_sema) == 0);
	ble_ll_ctrl_proc_start(connsm, BLE_LL_CTRL_PROC_CONN_UPDATE, NULL);
	return 0;
}

int blex_wait_conn_upd_end(void)
{
	return sema_wait(&conn_upd_sema);
}
#endif
...
#if RTBLE
void
ble_ll_conn_set_subrate_callback(ble_subrate_done_callback func)
{
	subrate_cb = func;
}

void
ble_ll_conn_set_base_tick(uint32_t t)
{
	base_tick = t;
	printf("base_tick = %ld\n", base_tick);
}

/* RT-BLE: we take advantage of connection subrating to
 * tweak the connection anchor point and virtual connection interval
 */
int
rt_ble_subrating_tweak(struct ble_ll_conn_sm *connsm,
							uint16_t subrate_factor,
							uint16_t subrate_base, uint16_t slot_num)
{
	struct ble_ll_conn_subrate_req_params srp;
	memset(&srp, 0, sizeof(struct ble_ll_conn_subrate_req_params));

	srp.subrate_min = subrate_factor;
	srp.subrate_max = subrate_factor;
	if (slot_num > 2) {
		srp.cont_num = 1;
	}
	else {
	...
	/*
	 * RT-BLE: The suepervision timeout should be increase with the retransmission number increase.
	 * To simplify the implementation, we juse set a large value.
	 */
//	srp.supervision_tmo = (connsm->conn_itvl * srp.subrate_max * (srp.max_latency + 1) *
//							BLE_LL_CONN_ITVL_USECS) * 20 / 10000;
	srp.supervision_tmo = 2000;
//	printf("subervision_tmo = %d\n", srp.supervision_tmo);
	if (srp.cont_num >= srp.subrate_max) {
        printf("cont num = %d, subrate_max = %d\n", srp.cont_num, srp.subrate_max);
		puts("s-3");
		return -EINVAL;
	}
...
#if MYNEWT_VAL(BLE_LL_ROLE_CENTRAL)
	case BLE_LL_CONN_ROLE_CENTRAL:
		connsm->subrate_trans.subrate_factor = subrate_factor;
		connsm->subrate_trans.subrate_base_event = subrate_base;
		connsm->subrate_trans.periph_latency = srp.max_latency;
		connsm->subrate_trans.cont_num = srp.cont_num;
		connsm->subrate_trans.supervision_tmo = srp.supervision_tmo;
		connsm->csmflags.cfbit.subrate_host_req = 1;
        printf(",subrate req send = %lu", os_cputime_ticks_to_usecs(os_cputime_get32()));
		ble_ll_ctrl_proc_start(connsm, BLE_LL_CTRL_PROC_SUBRATE_UPDATE, NULL);
		break;
#endif
#if MYNEWT_VAL(BLE_LL_ROLE_PERIPHERAL)
...
/* RT-BLE: use connection update to tweak the anchor point */
int rt_ble_conn_upd_tweak(struct ble_ll_conn_sm *connsm, int offset)
{
	struct hci_conn_update *hcu;
	if (!connsm) {
		return -1;
...
#if CONTINUOUS_LOSS
void ble_ll_conn_clear_continue(void)
{
    struct ble_ll_conn_sm *connsm;
    SLIST_FOREACH(connsm, &g_ble_ll_conn_active_list, act_sle) {
        connsm->loss_num_left = CONTINUOUS_LOSS_NUM;
...
#if RTBLE || BLEX
    base_tick = 0;
    sema_create(&conn_upd_sema, 0);
#endif
#if RTBLE
    subrate_cb = NULL;
#endif
    ll_connected_cb = NULL;
    sema_create(&output_sem, 0);
    sema_create(&txdone_sema, 0);
    sema_create(&tx_start_sema, 0);
}
#endif
