...
{
#if RTBLE
    struct ble_ll_conn_sm *connsm = (struct ble_ll_conn_sm *)sch->cb_arg;
#endif
    ...
    /* RT-BLE: if we are the continuation CE, we should not preempt other connection event */
#if RTBLE
    if (connsm->is_continuation) {
        return 0;
    }
#endif
...
#if RTBLE
static int ble_ll_sched_central_new_insert(struct ble_ll_sched_item *sch,
												uint32_t max_delay, uint32_t base_tick)
{
    struct ble_ll_sched_item *first;
    struct ble_ll_sched_item *entry;
    uint32_t max_start_time;
...
//	stdio_rtt_print_num(&connsm->conn_handle, 1, 16);
	...
	if (connsm->conn_handle == 1 && ble_ll_conn_active_connsm_num() == 1) {
		...
		if (connsm->offset % 2 == 0) {
			n = (sch->start_time - base_tick - 1) / 328 + 1;
		...
		sch->end_time = sch->start_time + 164;
	...
	if (connsm->conn_handle == 1 && ble_ll_conn_active_connsm_num() == 1) {
		...
		while (LL_TMR_GEQ(max_start_time, off_tick)) {
			if (max_delay != 0 && LL_TMR_GEQ(off_tick, sch->start_time) && LL_TMR_GEQ(max_start_time, off_tick)) {
				...
				sch->end_time = off_tick + 164;
				goto sch_insert;
			}
			off_tick += connsm->conn_itvl_ticks;
		}
		sch->enqueued = 0;
		goto done;
...
//	rtt_print_uint32(base_tick);
//	rtt_printf(", start_time = ");
//	rtt_print_uint32(sch->start_time);
//	rtt_printf(", cur_time = ");
//	rtt_print_uint32(os_cputime_get32());
	rc = ble_ll_sched_central_new_insert(sch, max_delay, base_tick);
#else
