...
#if RTBLE
#define CONNSM_STATE_NORMAL		1
#define CONNSM_STATE_CRITICAL	2
#endif
...
struct ble_ll_conn_sm
{
	...
	/* RT-BLE: we use these bytes to correct output the logs */
    uint8_t last_hdr_byte;
    ...
    // uint16_t last_data_event;

    uint8_t subrate_sema;
    ...
    union {
		...
		uint16_t critical_offset;
		uint16_t critical_slot_num;
        uint32_t peripheral_us;
		uint32_t subrate_anchor_point;
#if CONTINUOUS_LOSS
        uint8_t loss_num_left;
#endif
#endif

#if BLEX
		uint16_t offset;
		double expect_conn_event_len;  /* BLEX: the expected connection event length */
#endif
...
#if RTBLE
typedef void (*ble_subrate_done_callback)(uint16_t conn_handle);
#endif
typedef int (*ble_ll_connected_cb)(uint16_t conn_handle);

struct ble_ll_conn_sm * ble_ll_conn_find_by_handle(uint16_t handle);

void ble_ll_conn_set_connected_cb(ble_ll_connected_cb cb);
...
#if RTBLE
int rt_ble_subrating_tweak(struct ble_ll_conn_sm *connsm, uint16_t subrate_factor,
							uint16_t subrate_base, uint16_t slot_num);

void ble_ll_conn_set_base_tick(uint32_t t);

void ble_ll_conn_set_subrate_callback(ble_subrate_done_callback func);
